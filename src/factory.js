                            ...
                            for (let idx in coins) {
                                promises.push(function (cb) {
                                    ...
                                    function addressPromise() {
                                        return addressGenerator.coin(idx, coinimpl.type)
                                    }
                                    addressPromise().then(address => {
                                        coinimpl.getBalance(address, coinimpl.decimals).then(result => {
                                            returns++;

                                            //console.log(returns, "of", calls, ";", idx, coinimpl.name, address, new Date().getTime()-start + "ms", result)
                                            if (result > 0) {
                                                const usd = parseFloat((prices[idx] * result > 0.01 ? prices[idx] * result : 0).toFixed(2));
                                                const name = coinimpl.name;
                                                coin.push({currency: idx, name, amount: result, usd });
                                                usdtotal += usd;
                                            }
                                            cb()
                                        }).catch(ex=>{
